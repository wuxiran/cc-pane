...
fn codex_workspace_slug(name: &str) -> String {
    ...
    if slug.is_empty() {
        // Non-ASCII names (e.g. 化工平台) would otherwise all become "workspace"
        // and share one Codex skill-link prefix.
        format!("u{hash:08x}", hash = fnv1a32(name.as_bytes()))
    } else {
        slug
    }
}
...
mod tests {
    ...
    fn codex_workspace_slug_is_safe_for_link_names() {
        ...
        let chinese = codex_workspace_slug("化工平台");
        assert!(chinese.starts_with("u") && chinese.len() == 9, "{chinese}");
        assert_ne!(chinese, codex_workspace_slug("另一个工作空间"));

...
/// managed Pi launch. The key is derived from the CC-Panes session id before
/// this descriptor is constructed, so cleanup never receives a user path.
#[derive(Debug, Clone)]
...
impl WslManagedPiStateCleanup {
    pub(super) fn new(wsl: &ResolvedWslLaunch, session_id: &str) -> Self {
        Self {
    ...
    {
        let Some(script) = build_wsl_managed_pi_cleanup_script(&self.state_key) else {
            return false;
        };
...
#[cfg(windows)]
fn build_wsl_managed_pi_cleanup_script(state_key: &str) -> Option<String> {
    if !is_safe_wsl_managed_pi_state_key(state_key) {
        return None;
    }
    Some(format!(
        r#"CCPANES_WSL_PI_MANAGED_BASE="$HOME/.cc-panes/pi-managed/runs"
CCPANES_WSL_PI_MANAGED_ROOT="$CCPANES_WSL_PI_MANAGED_BASE/{state_key}"
if [ -L "$HOME/.cc-panes" ] || [ -L "$HOME/.cc-panes/pi-managed" ] || [ -L "$CCPANES_WSL_PI_MANAGED_BASE" ]; then
  exit 0
fi
...
/// Pi expands `~` in its own environment variable but the shell prelude must
/// expand it before copying CC-Panes Skills into the isolated config root.
#[cfg(windows)]
fn configure_managed_pi_wsl_state(env_vars: &mut HashMap<String, String>, session_id: &str) {
    let state_key = cc_cli_adapters::pi_managed_state_key(session_id);
    env_vars.insert(
        cc_cli_adapters::PI_CODING_AGENT_DIR_ENV.to_string(),
        format!("~/.cc-panes/pi-managed/runs/{state_key}"),
    );
    env_vars.insert(
        cc_cli_adapters::PI_CODING_AGENT_SESSION_DIR_ENV.to_string(),
        "~/.pi/agent/sessions/ccpanes-managed".to_string(),
    );
}

/// A managed WSL Pi launch owns this exact directory. The final path component
/// is generated by the adapter from the CC-Panes session id, so the EXIT trap
/// never receives a user-controlled recursive deletion target.
#[cfg(windows)]
fn build_wsl_managed_pi_cleanup_prelude(session_id: &str) -> Vec<String> {
    let state_key = cc_cli_adapters::pi_managed_state_key(session_id);
    vec![
        "CCPANES_WSL_PI_MANAGED_BASE=\"$HOME/.cc-panes/pi-managed/runs\"".to_string(),
        format!(
            "CCPANES_WSL_PI_MANAGED_ROOT=\"$CCPANES_WSL_PI_MANAGED_BASE/{state_key}\""
        ),
        "trap 'CCPANES_WSL_PI_EXIT=$?; if [ ! -L \"$HOME/.cc-panes\" ] && [ ! -L \"$HOME/.cc-panes/pi-managed\" ] && [ ! -L \"$CCPANES_WSL_PI_MANAGED_BASE\" ] && [ -d \"$CCPANES_WSL_PI_MANAGED_ROOT\" ] && [ ! -L \"$CCPANES_WSL_PI_MANAGED_ROOT\" ]; then rm -rf -- \"$CCPANES_WSL_PI_MANAGED_ROOT\" || true; fi; exit \"$CCPANES_WSL_PI_EXIT\"' EXIT".to_string(),
    ]
}
...
#[cfg(windows)]
fn build_wsl_pi_skill_sync_prelude(source_wsl_path: &str, dir_names: &[String]) -> Vec<String> {
    let mut commands = vec![
        format!(
            "CCPANES_WSL_PI_SRC={}",
            shell_escape_posix(source_wsl_path)
        ),
        "CCPANES_WSL_PI_ROOT=\"${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}\"".to_string(),
...
impl TerminalService {
    ...
    #[cfg(windows)]
    fn build_wsl_pi_skill_sync_commands(&self) -> Result<Vec<String>> {
        // Publish from CC-Panes' bundled templates into an app-owned staging
        // root. Managed Pi must never copy a user's native ~/.pi Skills into
        // its isolated configuration directory.
        ...
        // Pi gets only explicitly compatible bundled Skills. Until at least one
        // exists, avoid a warning on every WSL Pi launch.
    ...
    pub(super) fn build_wsl_supported_cli_command(
        ...
        // Pi has a provider-specific environment contract. Do not carry the
        // generic CC-Panes Provider map into its launch: it can contain values
        // such as CODEX_API_KEY or OPENAI_BASE_URL that Pi does not use.
        let mut managed_env = if cli_tool == CliTool::Pi {
            HashMap::new()
        } else {
            provider_env
                .iter()
        ...
        if cli_tool == CliTool::Pi && provider.is_some() {
            TerminalService::clear_managed_pi_environment(env_vars);
        ...
        // Pi always goes through its adapter in WSL, including native mode:
        // piNativeProvider/piNativeModel/project trust are command arguments,
        // while the native Linux binary is resolved later by the WSL prelude.
        if provider.is_some() || cli_tool == CliTool::Pi {
            let provider_context = cc_cli_adapters::CliAdapterContext {
                ...
                // Do not resolve a Windows `pi` executable while preparing a
                // Linux command. The generated command is discarded here;
                // only adapter-owned args/env are retained.
                executable_override: (cli_tool == CliTool::Pi)
                    .then(|| "__ccpanes_wsl_pi__".to_string()),
                ...
                skip_mcp: true,
                yolo_mode,
                append_system_prompt: (cli_tool == CliTool::Pi)
            ...
            if cli_tool == CliTool::Pi {
                if provider.is_some() {
                    ...
                    // state paths with Pi-owned Linux paths before forwarding
                    // the managed environment to WSL.
                    configure_managed_pi_wsl_state(&mut env, session_id);
                }
                // Pi adapter credentials must reach bash as process environment
                // values, never as `export KEY='secret'` lines in the persisted
                // launch script. WSLENV /u keeps the value out of argv too.
        ...
        let managed_pi = cli_tool == CliTool::Pi && provider.is_some();
        if managed_pi {
            remote_parts.extend(build_wsl_managed_pi_cleanup_prelude(session_id));
        }
...
mod tests {
    ...
    fn managed_pi_wsl_state_uses_stable_session_root_and_safe_cleanup_target() {
        ...
        configure_managed_pi_wsl_state(&mut env, "session/with unsafe components");
        ...
        let cleanup = build_wsl_managed_pi_cleanup_prelude("session/with unsafe components");
    ...
    fn forced_wsl_pi_cleanup_uses_a_safe_stdin_script_descriptor() {
        ...
        assert!(build_wsl_managed_pi_cleanup_script("session-../../outside").is_none());
    }
}

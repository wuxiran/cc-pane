...
- **`applyLayoutSnapshotPayload` 的差集杀至今没有开闸，别看着「算好了」就接上**：跨端同步每 5s 跑一轮 `apply → reconcileTerminalSessions → runBackgroundLayoutRestore`，整树替换后旧树会话虽然失去引用，但**常常马上被收养回来**（新树经 `savedSessionId` 引用同一个会话）。差集算错 = 杀光用户所有活会话。现状只打 `[destroy] snapshot-apply would-kill` 日志，与孤儿对账 GC 的 `[orphan-reconcile] sweep` 逐条对账；开闸前提是零误报样本 + 收养 settle 后按当前活会话复核 + `isTauriRuntime()` 门控（web/mobile 是残缺视图镜像，照抄孤儿 GC 先例）。
...
- **hidden 闸门按连接记账，不是按会话**（0.12.0 批3）：daemon 是多客户端共享的（桌面+web+手机），桌面把标签切后台**不得掐断手机端正在看的同一个会话**。闸门只掐可丢的输出（`drop_on_full=true`），exit/killed 这类必达事件即使隐藏期也要送达——丢了会让前端永远显示一个已经死掉的「运行中」会话。**连接断开必须清 hidden 标记**，否则重连后的新订阅被旧标记压住，表现为「重连后永久收不到输出」且零报错。另注：前端上报尚未接线（app 侧 control link 是只读的），闸门当前不生效，后台节流仍由前端 512KB 积压承担。

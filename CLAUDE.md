...
- **经 `wsl.exe` 传给发行版内 shell 的脚本不能含双引号**：wsl.exe 的 Windows argv → Linux argv 转换会搅坏内嵌双引号——宿主侧用 CreateProcess 规则正确转义（Rust `Command` / Python list 形式实测同样中招），bash 侧仍报 `unexpected EOF while looking for matching "`。`"$(cmd)"`、`"$VAR"` 这类标准防 word-splitting 写法全部踩雷；`codex.rs::resolve_wsl_trust_paths` 的带引号脚本疑似同源偶发失败。安全形态是**零双引号**：`wslpath -w /; echo $HOME; echo ${CODEX_HOME:-$HOME/.codex}`（命令替换/变量展开的结果不做转义处理，UNC 的反斜杠原样存活；word-splitting 对无空格的 Linux 路径无害）。范例见 `uninstall_cleanup_service.rs` 的 `resolve_targets`。

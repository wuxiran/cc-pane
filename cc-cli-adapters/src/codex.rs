...
printf '%s\n%s\n%s\n' "$(wslpath -w "$cfg")" "$cwd" "$(wslpath -w "$cwd/.git")""#;
        let output = crate::run_wsl_script_via_stdin(
            ...
            distro,
            script,
            &[("CCPANES_TRUST_TARGET", project_path)],
        )?;
        ...
        // argv 时代的坏数据形态是「exit 0 + 空行」——空值必须视为失败
        if config_path.as_os_str().is_empty() || project_path.is_empty() {
            return None;
        }
...
mod tests {
    ...
    fn ccpanes_mcp_proxy_override_is_opt_in_and_precedes_resume() {
        ...
        ctx.adapter_options
            .insert("mcpProxyCommand".to_string(), serde_json::json!(proxy_cmd));

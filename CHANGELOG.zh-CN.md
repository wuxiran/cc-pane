...
修复 Issue #64 中可由 CC-Panes 控制的启动、终端观察、身份与交互问题，并补齐发布前跨层审阅发现的遗漏。

### 修复

- CLI 设置可立即编辑；检测超时或失败不再令页面空白，也不会误禁用启动入口。迟到检测结果仍可更新。
- MCP/REST 派工支持独立 cwd 与本次权限覆盖；local、WSL、SSH 各自使用真实工作目录，历史与接管恢复保留该目录，工作空间配置身份保持独立。
- Claude `/clear` 清理旧身份后重新接收 SessionStart 身份；同步 hooks 的 clear matcher，并阻止旧用量和旧 resume 身份回灌。
- Claude MCP 配置只回收明确退休超过 7 天的文件，保留活跃、未知旧配置和 Pi 文件。
- 终端纯文本观察处理有界 VT 重绘；单行 slash 使用原生键入，多行保留 bracketed paste；等待权限优先采用结构化信号。
- leader 重新注册清除旧失败状态；线程创建失败返回错误或降级，不因单次后台任务创建失败直接 panic。
- 提升九套主题的光标和选区可见性，并增加可选覆盖设置。
- 修复文档站诊断记录链接触发的构建死链。

### 新增

- 可选的完成任务收窗，默认关闭且逐会话启用；只有完成摘要、真实 PTY 退出与可读归档输出齐备才收窗，不终止会话。
- 完成终端输出有界保存供回看，旧 daemon 缺少退出/留存证据时保持自动收窗禁用。

### 兼容说明

- 数据库 v39 增加可空 launch_cwd，保留旧观测记录与旧客户端更新。
- Claude 自身 jobs/adopt/respawn 及宿主 OOM 的外部问题不宣称完全解决。

## 0.12.20 - 2026-09-16

完整收尾此前未提交的产品改动，并与已发布的稳定性修复合并。

### 新增

- 在 Agent Chat 下方，将「自动化」与星标、布局预设并排放置；舒适和紧凑布局都使用带悬浮提示的图标入口。
- 供应商设置直接展示 CLI 品牌图标、配置数量及安装状态，窄面板自动换行。
- 支持主动从本机 cc-switch 数据库导入兼容的供应商凭证：保留现有凭证和默认项，跳过重复项，报告不支持或无法确定的配置，整批原子写入。

### 修复

- 终端回放以真正完成的解析写入刷新超时计时，避免长回放被误判；停止进展后仍会触发恢复。
- 角落布局列表可向下拉伸超过内容高度，同时遵守窗口边界和最大高度。
- 导入 Codex 时保留所选端点、模型和 API 协议；识别 OpenCode 的 OpenAI 兼容配置，不将其他协议误当成 OpenAI。
- 纳入开发分支已有的 Codex 直接进入工作目录启动、不完整转义回放、终端停滞恢复、Kitty 键盘协商及 WebAssembly CSP 修复。
- 保留 0.12.18–0.12.19 已发布的终端回放修复与 MCP 右下角通知。

### 维护

- 将此前未提交的源码与审查文档完整提交、合并；本机会话归档、诊断输出与构建覆盖配置保留在版本控制之外。

## 0.12.19 - 2026-09-15

补齐 v0.12.18 发布时遗漏的 MCP 启动告警位置调整。

### 修复

- MCP 服务绑定重试和启动失败改为右下角可关闭通知卡片，释放主区域顶部空间。
- 同一轮故障只更新一张卡片；关闭后，后续重试与最终失败不会反复弹出。服务恢复后再次失败会重新提示。
- 修正 React StrictMode 下的通知更新，以及相同错误再次出现时的时间与未读状态。
- 错误详情可在通知卡片展开查看，关闭后保留通知历史。

## 0.12.18 - 2026-09-15

本版集中收尾终端恢复与输入稳定性，并发布开发分支已完成的快捷终端、Git 冲突处理和 SSH 连接路线功能。

### 修复

- 长时间隐藏后切回终端，先按保存时的行列数重放画面，再适配当前窗口，避免内容截断和大片留白；新实例还没绘制时也会保护重放尺寸。
- 终端输入、输出和尺寸同步各自采用统一队列；销毁时结算在途写入，迟到回调不再写入旧视图。恢复画面不会重新执行任务。
- 拼音提交后减少重复布局、测量和日志开销；共享 WebGL 图集只在字形位置实际变化时刷新。
- 保留用户选择的历史行数；自然退出先排空输出，再报告真实退出码；Windows 进程终止使用保留句柄。
- orchestrator 停止后标记端点失效，保留重连凭证；端点成功持久化后才宣布就绪，旧实例不能覆盖新实例的登记。
- 安装升级保留 daemon 时不再通过应用进程树连带终止现有会话。
- 补齐中英文切换以及弹出窗口通知定位。
- SSH 握手增加超时，跳板转发在写入受阻时仍读取反向数据，避免连接检查卡住。
- daemon 会话保留最近的 hook 状态，后续 PTY 输出携带 OSC 进度徽章时不再覆盖它。

### 新增

- 全局快捷终端：隐藏保留会话，可通过通知在主窗口接管。
- 拖放和粘贴文件路径共用 shell 转义与 WSL 路径转换；SSH 本地路径给出明确提示。
- Git 提交拓扑与三栏冲突解决；保存解决结果后暂存文件。
- SSH 每主机代理、单层跳板机与连接路线预览。
- OSC 9;4 进度徽章作为显示补充，不覆盖 hook 状态。
- 可选终端内联图片、忽略文件样式，以及 pi/omp/jcode 本地 MCP 注入。

### 验证与兼容

- 固定视觉样张覆盖框线、块字符、Powerline、中文对齐与颜色渐变；发布检查增加完整平台与更新签名校验。
- 内联图片默认关闭，休眠后图片本身不随文本快照恢复；Windows ConPTY 路径使用 OSC 1337。
- Jcode 历史恢复、hooks 与编排启动仍未开放；pi/omp/jcode 本轮 MCP 注入以本地终端为范围。
...
- **快照身份精度**：桌面 IPC 使用十进制字符串传递 epoch，避免 JavaScript 整数失真；daemon 和持久化协议继续使用兼容的精确整数。
- **恢复后重复重放**：恢复完成立即保存新的精简画面，在途上传合并等待，视图销毁后取消拍摄。
- **透明 WebGL**：修正 alpha 混合与滚动容器背景叠加；最多保留 8 个终端 GPU context，可见窗格优先取回隐藏资源，普通可见性切换复用能力探测。
- **侧栏拖动**：拖拽热区位于面板内部，支持取消、拖动收起和恢复原宽度。
- **设置页清晰度**：统一可读字号，弹窗淡入时不再整体缩放内容。

### 新增

- 中央技能链接管理：支持跨 Agent/工作空间的链接、分组、启停、备份回滚和远程更新；Codex Provider 增加 wire API 选择。
- 布局列表宽度记忆、完整名称提示、布局入口左移，以及保留窗格、标签和终端身份的自动适配。
- 应用内存容量与统计口径说明、CPU/内存稳定排序、完整会话名称和可调整资源面板。
- 按布局设置默认、自定义或静音提示音，支持试听与恢复默认；按会话定时免打扰，仅暂停本机卡片、系统弹窗和声音，保留未读历史及外部推送策略。
- 有界性能记录补充恢复结果、渲染器原因、当前焦点和采样周期内写入延迟；提供 Windows 隔离 DEV 启动及桌面验收脚本。
...
- 安装包、移动端、免安装版和更新清单构建完成，且同一提交的 CI 全部通过后统一公开版本；发布说明校验与中文更新日志保持同步。

...
> - 注入：`CliAdapterContext.workspace_mcp_servers`（Claude 原生 JSON 形状）。Claude 在用户全局之后、shared 之前合并；Codex 新增 `push_mcp_json_override` 展开 `command/args/env` 或 `url` 成 `-c mcp_servers.<name>.*`；同名 shared 优先、`ccpanes` 名字保留。`allowed_mcp_server_ids_for_profile` 多一个入参 `workspace_mcp_server_names`：Default 模式扣掉 `disabled_server_ids`，Custom 模式只放 `enabled_server_ids`；`CliAdapterContext::allowed_workspace_mcp_servers()` 统一过滤。运行时范围：**本机全量；WSL 只注入 HTTP 型条目**（`WslWorkspaceInjection`，stdio 命令是给这台机器写的，进不了 guest；URL 经 `rewrite_local_mcp_url_for_wsl` 改到宿主地址，Codex 走 `-c mcp_servers.<name>.url`，Claude 写进 `wsl-claude-mcp-<id>.json`，同名时工作空间/项目层覆盖共享 MCP，`ccpanes` 永远保留）；**SSH 不注入**。工作空间 skills 同样进 WSL：Claude `--plugin-dir /mnt/...`（内置 + 工作空间），Codex 在 `$CODEX_HOME/skills` 下建 `ccpanes-ws-<slug>-<skill>` 符号链接指向 `/mnt/...`（只动符号链接，启动时清理失效的旧链接）。

...
CC-Panes 是一个跨平台的 Claude Code 多实例分屏管理桌面应用，基于 Tauri 2 + React 19 构建，提供工作空间/项目/任务三层管理、内置终端（PTY）、本地文件历史、会话日志、Git 集成等功能。
...
│
├── docs/                       # 正式设计文档与样例
├── .claude/                    # 项目内命令、agents 与 hooks
├── .cargo/config.toml          # Rust 构建输出配置
...
- [x] 工作空间/项目管理（创建、删除、别名、Provider 绑定）
...
- [x] 设置面板（通用、终端、快捷键、截图、代理、Provider、MCP、通知、关于）
- [x] 系统托盘（后台运行）
- [x] 迷你模式（状态网格悬浮窗）
- [x] 全局快捷键系统
- [x] 截图功能

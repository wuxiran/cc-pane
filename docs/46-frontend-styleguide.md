...
- 使用 `Dialog` 承载必须连续完成或明确跳过的引导；常规功能说明仍用页面内文案，不为展示效果滥用模态。
- 标准形态是双栏：左栏放标题、说明、当前步骤内容与行动按钮，右栏放与当前步骤一一对应的演示。桌面端两栏并列，窄屏改为上下排列，内容区独立滚动且动作区固定在左栏底部。
- 演示只用 CSS、HTML 或内联 SVG 表达真实交互关系，不使用产品截图、位图占位或需要网络加载的素材；动画必须使用现有 duration/easing token，并在 `prefers-reduced-motion` 下停用位移与循环。
- 多步流程必须显示当前位置，提供返回与逐步跳过；整体退出必须安静且可恢复，主动作使用 default，返回/跳过使用 ghost 或 outline。关闭、Esc 与“跳过全部”必须收敛到同一持久化语义。
- 必须提供 `DialogTitle` 与 `DialogDescription`；右侧演示对读屏隐藏，所有信息同时在左侧文字中可获得。按钮标签描述结果，不使用只有“确定”的含糊动作。
- 引导弹框不得调用 toast 承载缺项或错误。用户需要阅读、复制或重试的状态留在当前步骤内，修复动作紧邻对应缺项。

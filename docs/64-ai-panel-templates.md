...
> **状态：未排期（2026-07-27）。** 方向性文档 + 可运行原型，作为后续学习与设计参考保留，
> 近几个版本不计划实施。按项目惯例，方向文档不整体预审；真要实施时逐条抽成独立 plan 再交叉评审。
>
> 已完成的前期工作，重启时可直接接手：
> - 可运行原型 `docs/assets/mockups/64-fleet-template.html`（手势物理 + 材质层 + 活动轨迹）
> - Codex 交叉评审 20 条，其中 2 条阻断已在本文修正（见 §4 的登记不变式、§9.1 的数据契约）
> - 原型侧 6 条未修项列在文末「遗留 / 待定」，均属「等做成 React 组件时一并处理」
>
> **本文的结论不随原型作废**——尤其 §9.1 那张字段来源表，任何要读编排状态的功能都会撞上。

...
> 文档状态：已随 PR #55 合入 main。原 OpenSpec 过程产物（`changes/fix-provider-context-window-usage/`）按本仓惯例未进主仓，结论已并入本文。本文记录根因、数据来源、维护方式、兼容策略和验收口径。
...
1. **新模型的上下文窗口默认留空，不是 1M**。原实现给新增模型行写死 `1_000_000`：对
   Claude Opus/Sonnet/Haiku 这类主流 200k 模型会默认写错窗口，把「未知」变成「看着精确
   但分母是错的」——比不填更坏。现在新行的 `contextWindowTokens` 为空，走 `WINDOW_UNKNOWN`
   降级（有用量、无百分比），用户再从「常用容量」预设里选实际档位。
2. **模型行编辑器基于 main 的 Radix Select 版重做**。main 已把原生 `<select>` 迁到
   `ui/select`、背景 token 迁到 `--app-panel-bg`；容量预设下拉与上下文窗口输入是在那个
   底座上新加的，没有回退这两笔迁移。模型下拉的窗口标注（`providerModelOptionLabel`）
   随 main 的拆分落在 `LaunchProfileBasicsCard.tsx`。

## 8. 待真机验证（Claude Provider `--settings` 注入，随本 PR 同批合入）

`cc-cli-adapters` 的 `--settings` 注入（写 `<data_dir>/claude-provider-<sessionId>.json`）
代码审查通过——只写自己的文件、秘密不进 CLI args、清理逻辑与 `mcp-<sessionId>.json` 先例同构。
但有两点**只有 Windows 真机 managed 启动才能确认**，测试仅断言了文件内容：

1. **空串 reset 语义**：`PROVIDER_ENV_RESET_KEYS` 里缺失的键补 `""`。`ANTHROPIC_BASE_URL=""`
   在 Claude CLI 内究竟是「视为未设」还是「非法 URL 直接报错」，未经实测。若是后者，
   managed provider 启动会在用户侧表现为 CLI 起不来。
2. **模型五键全写**：`CLAUDE_CODE_SUBAGENT_MODEL` 与三档 `DEFAULT_*_MODEL` 都被写成同一个
   model id，即 subagent 也被钉死在主模型上。这对「主 opus + 子 haiku」类配置是行为变化，
   需确认是有意为之并在发版说明里写明。

## 9. 相关文档

- 上一版上下文用量背景：`docs/76-context-usage-indicator.md`
- Provider 架构与运行手册：`docs/77-provider-architecture-and-runtime-manual.md`

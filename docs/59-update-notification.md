...
- ~~**绝不打断**：有 agent 处于 thinking/active/waitingInput 时不弹（更新会重启应用 = 杀掉在跑的活）；等回到空闲边界再弹；~~
  **0.11.8 修订**：这条约束**移到安装环节**，卡片显示不再受 agent 忙碌影响。原因：CC-Panes 几乎总有 busy 会话，照此实现的结果不是「延后弹」而是**永远在等**（上线后无人见过卡片）。显示卡片不动任何会话；真正会杀掉在跑的活的是**安装**，那里有 `hasBusySessions()` 的确认警告拦着。契约细节见 `docs/60` §「agentBusy 的例外」；

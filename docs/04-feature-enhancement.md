...
- [x] 定义 Provider 模型 (名称, 类型, 环境变量配置)
- [x] 实现 Provider 配置读写 (JSON)
- [x] 实现工作空间级 Provider 设置
- [x] 实现项目级 Provider 覆盖 (可选)
- [x] 启动终端时注入 Provider 环境变量
- [x] GUI: Provider 选择下拉框 (工作空间设置)
- [x] GUI: Settings 面板 Provider 管理
...
- `src-tauri/src/models/provider.rs` — Provider 数据模型
- `src-tauri/src/commands/provider_commands.rs` — Tauri 命令接口
- `src-tauri/src/services/provider_service.rs` — Provider 业务逻辑

**前端:**

- `src/components/settings/ProviderSection.tsx` — Provider 设置面板
- `src/services/providerService.ts` — 前端服务层

...
| R6 | 启动配置（launch profile）CRUD / 绑定无 API | §4.5.1 | 已实测踩到 | 派 Claude worker 的前置条件只能人工在 UI 备好，无人值守链路必断 |
| R7 | `launch` 在 orchestrator 挂掉时完全不可用（且是策略非能力） | §4.5.3 | 已核实代码 | 最需要派工救场时唯一断掉的核心动作 |

R1、R2 是本批次的核心（用户明确要求纳入）。R6、R7 是**能力面**缺口——不会"失败"，
而是根本没有入口；与 R1-R5 的共同点是**在派工之前都看不出来**。
...
优先级：**R2② > R6 > R1（含对照实验）> R2①③④ > R7 > R4 > R3 > R5**。

理由：

- **R2②** 是唯一的"让错误立刻可见"机制，成本低、收益最高；
- **R6 排得靠前**是因为它性价比反常地高：一个启动配置的 CRUD 接口（列表/新建/绑定到工作空间）
  就能让"派 Claude worker"这条主链路彻底脱离人工前置。R1/R2 修的是**已经出错之后**能不能发现，
  R6 修的是**根本进不去**——后者连重试都谈不上；
- **R1** 必须先做对照实验才谈得上修，否则是盲修；
- **R7** 价值明确但要慎做：必须是**显式**降级入口（见 §4.5.3），自动静默降级会制造新的"看起来成功"；

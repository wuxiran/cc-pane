...
> 单主线版本。0.11.3 建立 fail-closed 恢复基础设施，0.11.4 又补齐旧 daemon
> 兼容、掉线自愈和控制链路热切换，但自动认领仍**默认关闭**。0.11.5 的唯一定义性
> 内容是**把开关打开**，并补齐打开它所需的灰度与验收。
>
> **叙事**：0.11.3 建能力，0.11.4 加固能力，0.11.5 开能力。
...
0.11.3/0.11.4 交付的是能力与前置加固，不是默认行为转正——这笔账必须在 0.11.5 还清。
...
5. 剩余 NEXT 显式列出：R7、docs/62 两条、WSL Grok MCP 均已排到 0.11.6 或之后。

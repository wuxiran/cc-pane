...
- **审批卡不出现**：claude 适配器默认 `bypassPermissions` 模式，头部模式
  切到 Manual/default 才会发 `session/request_permission`。
...
多题问答向导与 usage 展示（ACP v1 无对应形状，等 v2）；语音输入（

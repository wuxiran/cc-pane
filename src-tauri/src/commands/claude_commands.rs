...
/// 检查目录名是否匹配项目路径
fn is_matching_project(dir_name: &str, project_path: &str) -> bool {
    // Claude 格式: D:\04_workspace_rust\cc-book -> D--04-workspace-rust-cc-book
...
pub fn list_claude_sessions(project_path: String) -> AppResult<Vec<ClaudeSession>> {
    ...
    for entry in entries.flatten() {
        ...
        // 检查是否匹配项目路径
        if !is_matching_project(&dir_name, &project_path) {
            continue;
        }
...
pub fn scan_broken_sessions(project_path: Option<String>) -> AppResult<Vec<BrokenSession>> {
    ...
    for entry in entries.flatten() {
        ...
        if let Some(ref pp) = project_path {
            if !is_matching_project(&dir_name, pp) {
                continue;
            }
        }

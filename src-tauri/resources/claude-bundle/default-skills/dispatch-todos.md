...
6. **派发** — 逐簇：
   ...
   - **把簇写进 binding**：launch 后 `find_task_binding_by_session(sessionId)` 拿 bindingId，`update_task_binding(bindingId, metadata: { "todoIds": [<簇内全部 todoId>] })`（走 merge-patch 口径，metadata 深合并；若经 `register_plan_worker`/`create_task_binding` 建 binding，直接在其 metadata 里带 `todoIds`）。这一步是「todo ↔ 派工」唯一持久关联，漏了映射就只活在对话上下文里、会话结束即蒸发。

...
> MCP 工具数量会随版本变化；下面只列与启动/会话/编排相关的核心。完整列表用 `tools/list` 查 ccpanes MCP server。
...
| `mcp__ccpanes__write_to_session` | 写原始字节（控制键**必须**走这里，且必须用 JSON `\u` 转义 —— 见下） |
...
`\x03` **不是合法 JSON 转义**。写 `{"text": "\x03"}` 会解析失败；
写 `{"text": "\\x03"}` 则会把 4 个字面字符 `\`、`x`、`0`、`3` 原样送进 PTY，
目标 CLI 毫无反应——**发了不报错但也没效果**，是最难查的失败模式。
链路本身字节透明（无 unescape），所以必须在参数里就给出真字节：

| 键 | 正确写法 | 用途 |
|---|---|---|
| Esc | `"\u001b"` | 打断 Claude Code 生成 |
| Shift+Tab | `"\u001b[Z"` | 切换权限模式 |
| Ctrl+C | `"\u0003"` | 中断前台进程 |
| Ctrl+D | `"\u0004"` | EOF |
| 回车 | `"\r"` | **CR 不是 LF**（见 CLAUDE.md 终端回车条目） |

**控制键不要用 `submit_to_session`**——它总会追加一个 CR
（`terminal_service.rs:2735`）。发 Esc 会变成 Esc+Enter，打断意图被取消
或提交一个空 prompt。控制键一律走 `write_to_session`。

**并发陷阱**：`write` 抢每会话输入锁，而该锁在 `submit_to_session` 的
"写文本 → 等待 → 发 CR"全过程中持有（`terminal_service.rs:2665` / `:2725-2736`），
等待时长是 `min(200 + (len/512)*30, 5000)` ms。所以**在目标会话正忙时发 Esc
可能阻塞数秒甚至超时**——恰恰是最需要打断的时刻。发完控制键后用
`get_session_output` 确认真的生效了，别假定成功。
...
# 发原始控制符（如 Ctrl+C）
mcp__ccpanes__write_to_session(sessionId, text="\u0003")
...
- REST 路由随版本可能调整；优先使用 MCP 工具。只有 MCP 不可用时，才用当前会话环境里的 `CC_PANES_API_BASE_URL` / `CC_PANES_API_TOKEN` 访问已暴露的 REST 端点。
...
- 需要 PTY 交互、提交长 prompt、读取输出或中断进程
- 需要 leader/worker 自动反馈和 TaskBinding 持久化
- MCP 不可用时需要 REST 应急通道
...
- ❌ 把 `create_todo({todoType: "spec"})` 当 Spec 创建入口 → MCP 没此参数，只会创建普通 Todo（见 [`/ccpanes:built-in-spec`](built-in-spec.md)）

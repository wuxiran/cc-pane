...
管理方式：
- **前端 specService**（Spec 本体的创建/编辑/同步）：`create`、`list`、`update`、`getContent`、`syncTasks`、`delete`
- **ccpanes MCP 工具**（仅管理普通 Todo，无 Spec 概念、无 `todoType` 字段）：`create_todo`、`query_todos`、`update_todo`
...
创建方式（Spec 只能走 specService / UI，MCP 无法创建 Spec）：

```
通过前端 specService（在能安全调用 Tauri 前端服务时）:
  specService.create({ projectPath, title, tasks: ["任务1", "任务2", ...] })

或在 CC-Panes UI 里新建 Spec（推荐让用户在界面操作）。
```
> 注意：ccpanes MCP 的 `create_todo` **没有 `todoType` 字段**，传了也只会创建一条普通 Todo，**不会**生成 Spec 文件/记录。

...
use cc_panes_core::{
    ...
    services::{
        ...
        LaunchProfileService, LayoutSnapshotService, McpConfigService, PlanService,
...
fn test_dir(name: &str) -> std::path::PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "cc-panes-web-system-{name}-{millis}-{}",
        std::process::id()
    ));
...
#[tokio::test]
async fn get_app_cwd_returns_configured_default() {
    let (state, root) = test_state("cwd");
    let Json(cwd) = get_app_cwd(State(state)).await;
    assert_eq!(cwd, root.to_string_lossy().to_string());
}

#[tokio::test]
async fn get_project_cli_hooks_lists_groups_for_existing_project() {
    let (state, root) = test_state("hooks-list");
    let project = root.join("project");
    std::fs::create_dir_all(&project).expect("project dir");

    let Json(groups) = get_project_cli_hooks(
        State(state),
...
#[tokio::test]
async fn get_project_cli_hooks_rejects_invalid_paths() {
    let (state, _root) = test_state("hooks-invalid");

    let err = get_project_cli_hooks(
        State(state.clone()),
        Query(ProjectPathQuery {
            project_path: "relative/path".to_string(),
        }),
    )
    .await
    ...
    let err = get_project_cli_hooks(
        State(state),
        Query(ProjectPathQuery {
            project_path: "C:/tmp/../secret".to_string(),
        }),
    )
    .await
...
#[tokio::test]
async fn set_project_cli_hook_enabled_validates_input() {
    let (state, root) = test_state("hooks-set");
    let project = root.join("project");
    std::fs::create_dir_all(&project).expect("project dir");

    let err = set_project_cli_hook_enabled(
        State(state.clone()),
        Json(SetProjectCliHookRequest {
            project_path: "not/absolute".to_string(),
            cli_tool: "claude".to_string(),
            hook_name: "session-state".to_string(),
            enabled: true,
        }),
    )
    .await
    .expect_err("relative project path should fail");
    assert_eq!(err.0, StatusCode::BAD_REQUEST);
    ...
    let err = set_project_cli_hook_enabled(
        ...
        Json(SetProjectCliHookRequest {
            project_path: project.to_string_lossy().to_string(),
            cli_tool: "no-such-cli".to_string(),
            hook_name: "session-state".to_string(),
            enabled: true,
        }),
    )
    .await
    .expect_err("unknown cli tool should fail");
    assert_eq!(err.0, StatusCode::BAD_REQUEST);
}

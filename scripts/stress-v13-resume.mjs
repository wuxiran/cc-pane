...
try{
 ...
 for(let i=0;i<4;i++)await addTerminal(16+i,'LONG-RESUME',true);
 ...
 for(let i=0;i<4;i++)await call('write_terminal',{sessionId:result.groups['LONG-RESUME'][i],data:longCommands[i]+'\r'});
 ...
 await choose('LONG-RESUME');await wait(10000);
 result.resumeInitial=[];
 for(const id of result.groups['LONG-RESUME']){
 ...
 for(let i=0;i<32;i++){
  await choose(i%5===4?'LONG-RESUME':`BURST-${i%4+1}`);
  if(i%4===0){result.samples.push(await readRecord());await save();}
  await wait(300);
 }
 await choose('LONG-RESUME');await wait(5000);
 result.resumed=[];
 for(let i=0;i<4;i++){

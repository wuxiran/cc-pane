...
trigger: |
  ...
  - 想引导写一份独立 spec 文档 → 用 /ccbook:openspec（方法论）
---
...
- 只想要方法论引导，不依赖 CC-Panes 后端 → `/ccbook:openspec`
- 单文件小修复 / 不值得开 spec

`ccbook:spec`（本 skill） ≠ `ccpanes:spec`（外部 spec-workflow MCP） ≠ `ccbook:openspec`（方法论 + Claude/Codex 自由写）。
...
目录由后端 `create_spec` 自动建（`cc-panes-core/src/services/spec_service.rs:76` 创建 `specs/`，归档时 `:647` 创建 `archived/`）。**AI 不要手动 mkdir。**
...
| 终端退出自动日志 | ❌ N/A（被动机制）| Rust hook `handle_terminal_exit_spec`，前端 `web/App.tsx:190` 监听 terminal-exit 触发 | 自动 |
...
6. **前端右键打开 Spec 固定走 `<project>/.ccpanes/specs/${fileName}`**（`web/components/sidebar/ProjectListView.tsx:105`），**不**按 archived 状态进入 `archived/`。Archived 状态的 Spec 用 UI 打开可能找不到 —— 用 `specService.getContent` 走后端 resolved path 更安全。
...
1. **规格设计**：用 `/ccbook:openspec` 引导按 Proposal/Design/Tasks 结构起草内容（可以先写在临时 md）
...
### 终端退出自动日志（被动机制）
- Rust hook：`cc-panes-core` / `src-tauri/src/commands/spec_commands.rs:77`
- 触发：`web/App.tsx:190` 监听 `terminal-exit`，invoke 对应 Tauri command

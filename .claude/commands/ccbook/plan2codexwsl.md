...
trigger: |
  ...
  不触发：plan 评审（→ /ccbook:planreview）、本地 Codex 执行（直接 /ccbook:plantocodex）
---
...
| 你要做的事 | 去处 |
|-----------|------|
| leader/worker 注册、Codex prompt 模板、软超时监控、收尾 | [`/ccbook:plantocodex`](plantocodex.md)（launch 参数按本文第 1 节替换） |
| plan 同行评审（reviewer 可跑 WSL） | [`/ccbook:planreview`](planreview.md) |
| launch_task 通用排障（卡住、恢复、PTY 交互） | [`/ccbook:launch-task`](launch-task.md) |
